...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.get('/v1/rates/btcz', cache('15 minutes'), function(req, res, next) {
    console.log('Pulling BTCZ Rate information from APIs');
    btczRates.getAll().then(function (rates) {
      res.json(rates);
    }).catch(next);
  });
